...
namespace SCPE
{
    ...
    public class PostEffectRenderer<T> : ScriptableRenderPass
    {
        ...
        public bool ShouldRender(RenderingData renderingData)
        {
            ...
            if (settings.allowedCameraTypes != EffectBaseSettings.AllowedCameraType.Both && renderingData.cameraData.camera.cameraType != CameraType.SceneView)
            {
                if (renderingData.cameraData.renderType == CameraRenderType.Overlay &&
                    settings.allowedCameraTypes != EffectBaseSettings.AllowedCameraType.Overlay) return false;
                if (renderingData.cameraData.renderType == CameraRenderType.Base &&
                    settings.allowedCameraTypes != EffectBaseSettings.AllowedCameraType.Base) return false;
            }

            //Camera depth/priority mask?
            
            #if UNITY_EDITOR
            if (renderingData.cameraData.camera.cameraType == CameraType.SceneView)
            {
                if (!settings.allowInSceneView) return false;
        ...
        protected void CopyTargets(CommandBuffer cmd, RenderingData renderingData)
        {
            ...
            //TODO: Check if has already been rendered by another effect in the stack
        ...
        /// <summary>
        /// Computes a clip-space to world-space projection, used for depth-ray to world direction reconstruction
        /// </summary>
        /// <param name="camera"></param>
        /// <returns></returns>
        protected static Matrix4x4 ClipToWorldMatrix(Camera camera)
        {
        ...
        /// </summary>
        /// <param name="cmd"></param>
        protected void CopyDepth(ScriptableRenderPass pass, CommandBuffer cmd)
        {
#if DEPTH_COPY
        ...
        protected void FinalBlit(ScriptableRenderPass pass, ScriptableRenderContext context, CommandBuffer cmd, RenderTargetIdentifier source, RenderTargetIdentifier dest, Material mat, int passIndex)
        {
        ...
        protected void ToggleStackedCamerasKeyword(Material mat, bool state)
        {
#if UNITY_EDITOR || DEVELOPMENT_BUILD
            if (!mat)
            {
