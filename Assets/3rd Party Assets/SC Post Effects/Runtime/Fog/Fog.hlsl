...
float4 GetFogColor(float2 screenPos, float3 worldNormal, float distanceFactor)
{
    #if !defined(SHADERGRAPH_PREVIEW)
    ...
    float NdotL = max(0, dot(worldNormal, _DirLightParams.xyz));
...
float GetFogHeight(float3 worldPos, float time, float skyboxMask)
{
    ...
    if (_DistanceParams.w == 1) { //Heightfog enabled
        ...
        {
            noise = SAMPLE_TEX(_NoiseTex, Repeat, worldPos.xz * _NoiseParams.x + (time * _NoiseParams.y * float2(0, 1))).r;
            noise = lerp(1, noise, _DensityParams.y * skyboxMask);
        }
#endif
...
void ApplyFog_float(in float3 worldPos, in float3 worldNormal, in float4 screenPos, in float time, in float4 color, in float3 emission, out float4 outColor, out float3 outEmission)
{
    ...
    float4 fogColor = GetFogColor(screenPos.xy, worldNormal, distanceFog);
